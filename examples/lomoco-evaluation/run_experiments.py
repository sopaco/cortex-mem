...
from src.cortex_mem.add import CortexMemAdd
from src.cortex_mem.search import CortexMemSearch
...
def main():
    ...
    elif args.technique_type == "cortex_mem":
        if args.method == "add":
            cortex_mem_manager = CortexMemAdd(data_path="dataset/locomo10.json")
            cortex_mem_manager.process_all_conversations()
        elif args.method == "search":
            output_file_path = os.path.join(args.output_folder, "cortex_mem_results.json")
            cortex_mem_searcher = CortexMemSearch(output_file_path, args.top_k)
            cortex_mem_searcher.process_data_file("dataset/locomo10.json")
    else:
