...
mod tests {
    use super::*;
    use crate::retrieval::intent::{Intent, QueryType};
    
    #[test]
    fn test_keyword_matching() {
        let calculator = RelevanceCalculator::new();
        
        let text = "OAuth 2.0 is an authentication protocol widely used for secure authorization.";
        let intent = Intent {
            keywords: vec!["oauth".to_string(), "authentication".to_string()],
            entities: vec![],
            time_range: None,
            query_type: QueryType::General,
        };
        
        let score = calculator.calculate(text, &intent);
        assert!(score > 0.5, "Score should be high for matching keywords");
    }
    ...
    fn test_entity_matching() {
        ...
        let text = "OpenAI developed GPT-4, a large language model.";
        let intent = Intent {
            keywords: vec!["language".to_string(), "model".to_string()],
            entities: vec!["OpenAI".to_string(), "GPT-4".to_string()],
            time_range: None,
            query_type: QueryType::General,
        };
        
        let score = calculator.calculate(text, &intent);
        assert!(score > 0.7, "Score should be very high for matching keywords and entities");
    }
    ...
    fn test_no_match() {
        ...
        let text = "This is about completely different topic.";
        let intent = Intent {
            keywords: vec!["oauth".to_string(), "authentication".to_string()],
            entities: vec![],
            time_range: None,
        ...
        let score = calculator.calculate(text, &intent);
        assert!(score < 0.4, "Score should be low for non-matching text, got {}", score);
    }
    ...
    fn test_bm25_calculation() {
        ...
        let text = "OAuth OAuth authentication security";
        let keywords = vec!["oauth".to_string(), "authentication".to_string()];
        
        let score = calculator.calculate_bm25(text, &keywords, 10.0, 1.5, 0.75);
        assert!(score > 0.0, "BM25 score should be positive for matching keywords");
    }
}
