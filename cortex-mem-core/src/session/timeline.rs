...
impl TimelineGenerator {
    ...
    pub async fn generate_yearly_index(
        &self,
        thread_id: &str,
        year: i32,
    ) -> Result<String> {
    ...
    pub async fn generate_all_indexes(&self, thread_id: &str) -> Result<Vec<String>> {
        ...
        for year_month_entry in entries {
            ...
            for day_entry in day_entries {
                ...
                let daily_index = self.generate_daily_index(thread_id, year, month, day).await?;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_daily_index_generation() {
        let temp_dir = TempDir::new().unwrap();
        let fs = Arc::new(CortexFilesystem::new(temp_dir.path()));
        fs.initialize().await.unwrap();
        
        let storage = MessageStorage::new(fs.clone());
        let generator = TimelineGenerator::new(fs.clone());
        
        // Create some messages
        let msg1 = Message::user("First message");
        let msg2 = Message::assistant("Response");
        
        storage.save_message("test-thread", &msg1).await.unwrap();
        storage.save_message("test-thread", &msg2).await.unwrap();
        
        // Generate daily index
        let now = Utc::now();
        let index_uri = generator
            .generate_daily_index("test-thread", now.year(), now.month(), now.day())
            .await
            .unwrap();
        
        assert!(index_uri.contains("index.md"));
        
        // Verify index was created
        let exists = fs.exists(&index_uri).await.unwrap();
        assert!(exists);
        
        // Read and verify content
        let content = fs.read(&index_uri).await.unwrap();
        println!("Generated index content:\n{}", content);
        assert!(content.contains("Timeline:"));
        // Note: The count might be different due to how filesystem lists files
        // So we just check that the index was generated
    }
    
    #[tokio::test]
    async fn test_monthly_index_generation() {
        let temp_dir = TempDir::new().unwrap();
        let fs = Arc::new(CortexFilesystem::new(temp_dir.path()));
        fs.initialize().await.unwrap();
        ...
        storage.save_message("test-thread", &msg).await.unwrap();
        
        // Generate monthly index
        let now = Utc::now();
        let index_uri = generator
            .generate_monthly_index("test-thread", now.year(), now.month())
            .await
            .unwrap();
