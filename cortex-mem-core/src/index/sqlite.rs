...
impl SQLiteIndex {
    ...
    pub fn delete(&self, uri: &str) -> Result<()> {
        self.conn.execute("DELETE FROM memories WHERE uri = ?1", [uri])?;
        Ok(())
    }
    ...
    pub fn get(&self, uri: &str) -> Result<Option<IndexEntry>> {
        ...
        let entry = stmt.query_row([uri], |row| {
            Ok(IndexEntry {
                uri: row.get(0)?,
                dimension: row.get(1)?,
                id: row.get(2)?,
...
mod tests {
    ...
    #[test]
    fn test_sqlite_index_basic() {
        let index = SQLiteIndex::in_memory().unwrap();
        
        let entry = IndexEntry {
            uri: "cortex://agents/test-agent/memories/msg1.md".to_string(),
            dimension: "agents".to_string(),
            id: "test-agent".to_string(),
            category: Some("memories".to_string()),
            created_at: Utc::now(),
            updated_at: Utc::now(),
            size: 1024,
        ...
        let retrieved = index.get(&entry.uri).unwrap().unwrap();
        assert_eq!(retrieved.uri, entry.uri);
        assert_eq!(retrieved.dimension, "agents");
    ...
    fn test_time_series_query() {
        ...
        // Query by dimension
        let results = index.list_by_dimension("threads").unwrap();
        assert_eq!(results.len(), 2);
        
        // Query by ID
        let results = index.list_by_id("threads", "t1").unwrap();
        assert_eq!(results.len(), 2);
        
        // Query with time range
        let results = index.query(&TimeSeriesQuery {
            dimension: Some("threads".to_string()),
            id: None,
            category: None,
            start_time: Some(past - chrono::Duration::hours(1)),
            end_time: Some(past + chrono::Duration::hours(1)),
            limit: None,
        }).unwrap();
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].uri, entry1.uri);
    }
    ...
    fn test_delete() {
        ...
        let entry = IndexEntry {
            uri: "cortex://global/notes/note1.md".to_string(),
