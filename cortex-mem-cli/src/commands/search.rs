...
#[cfg(feature = "vector-search")]
use cortex_mem_core::{
    search::SearchOptions, EmbeddingClient, QdrantVectorStore, VectorSearchEngine,
};
...
pub async fn execute(
    ...
    match engine {
        SearchEngine::Keyword => search_keyword(fs, query, thread, limit, min_score).await,
        #[cfg(feature = "vector-search")]
        SearchEngine::Vector => search_vector_mode(fs, query, thread, limit).await,
        #[cfg(feature = "vector-search")]
        SearchEngine::Hybrid => search_hybrid_mode(fs, query, thread, limit).await,
        #[cfg(feature = "vector-search")]
        SearchEngine::Layered => search_layered_mode(fs, query, thread, limit).await,
    }
}
...
async fn search_vector_mode(
    ...
    println!(
        "  {} Strategy: {}",
        "⚙".dimmed(),
        "Vector (Semantic)".bright_magenta()
    );

    // Load configs
    if !Path::new("config.toml").exists() {
        anyhow::bail!(
            "config.toml not found. Vector search requires Qdrant and embedding configurations."
        );
    }
    ...
    // Initialize clients
    println!(
        "  {} Initializing Qdrant and embedding clients",
        "🔌".dimmed()
    );
    ...
    let qdrant =
        Arc::new(QdrantVectorStore::new_with_llm_client(&qdrant_config, &llm_client).await?);
...
async fn search_hybrid_mode(
    ...
    println!(
        "  {} Strategy: {}",
        "⚙".dimmed(),
        "Hybrid (Keyword + Vector)".bright_magenta()
    );

    // Load configs and initialize
    if !Path::new("config.toml").exists() {
        anyhow::bail!(
            "config.toml not found. Hybrid search requires Qdrant and embedding configurations."
        );
    }
    ...
    let keyword_result = keyword_engine
        .search(query, &scope, keyword_options)
        .await?;
    ...
    let vector_results = vector_engine
        .semantic_search(query, &vector_options)
        .await?;
...
async fn search_layered_mode(
    ...
    println!(
        "  {} Strategy: {}",
        "⚙".dimmed(),
        "Layered Vector (L0→L1→L2)".bright_magenta()
    );

    // Load configs
    if !Path::new("config.toml").exists() {
        anyhow::bail!(
            "config.toml not found. Layered search requires Qdrant and embedding configurations."
        );
    }
...
fn load_llm_config_from_toml(path: &str) -> Result<cortex_mem_core::llm::client::LLMConfig> {
    ...
    let config = cortex_mem_core::llm::client::LLMConfig {
        ...
        api_key: llm_section
            ...
            .to_string(),
        model_efficient: llm_section
            .get("model_efficient")
            .and_then(|v| v.as_str())
            .ok_or_else(|| anyhow::anyhow!("Missing llm.model_efficient"))?
            .to_string(),
        temperature: llm_section
            .get("temperature")
            .and_then(|v| v.as_float())
            .unwrap_or(0.1) as f32,
        max_tokens: llm_section
            .get("max_tokens")
...
#[cfg(feature = "vector-search")]
fn load_vector_configs(
    path: &str,
) -> Result<(cortex_mem_core::config::QdrantConfig, EmbeddingConfig)> {
    let content = std::fs::read_to_string(path)?;
    let value: toml::Value = toml::from_str(&content)?;

    // Parse Qdrant config
    let qdrant_section = value
        .get("qdrant")
    ...
    let qdrant_config = cortex_mem_core::config::QdrantConfig {
        url: qdrant_section
            ...
            .to_string(),
        collection_name: qdrant_section
            .get("collection_name")
            .and_then(|v| v.as_str())
            .ok_or_else(|| anyhow::anyhow!("Missing qdrant.collection_name"))?
            .to_string(),
        embedding_dim: qdrant_section
            .get("embedding_dim")
            .and_then(|v| v.as_integer())
            .map(|n| n as usize),
        timeout_secs: qdrant_section
            .get("timeout_secs")
            .and_then(|v| v.as_integer())
            .unwrap_or(30) as u64,
    };
