<script lang="ts">
  ...
  function getStatusColor(status: string) {
    switch (status) {
      case 'connected': return 'text-green-500 bg-green-50 dark:bg-green-900/20';
      case 'connecting': return 'text-yellow-500 bg-yellow-50 dark:bg-yellow-900/20';
      case 'disconnected': return 'text-red-500 bg-red-50 dark:bg-red-900/20';
  ...
  function getImportanceColor(importance: number) {
    ...
    if (importance >= 0.7) return 'bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300';
    if (importance >= 0.5) return 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300';
  ...
  {:else}
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border-l-4 border-blue-500">
        ...
        <p class="mt-4 text-sm text-gray-500 dark:text-gray-400">
          今日新增: <span class="font-medium text-green-600 dark:text-green-400">+{stats.todayAdded}</span>
        </p>
      </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border-l-4 border-purple-500">
        ...
        <p class="mt-4 text-sm text-gray-500 dark:text-gray-400">
          平均质量: <span class="font-medium text-blue-600 dark:text-blue-400">{(stats.averageQuality * 100).toFixed(1)}%</span>
        </p>
      </div>
            ...
            {#each Object.entries(systemStatus) as [service, status]}
              <div class="flex items-center justify-between p-3 rounded-lg bg-gray-50 dark:bg-gray-900/50">
                <div class="flex items-center space-x-3">
                  <div class={`w-3 h-3 rounded-full ${status === 'connected' ? 'bg-green-500' : status === 'connecting' ? 'bg-yellow-500' : 'bg-red-500'}`}></div>
                ...
                <span class={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(status)}`}>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
          ...
          <div class="space-y-4">
            {#each recentMemories as memory}
              <div class="p-4 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors duration-200">
                <div class="flex items-start justify-between">
                  <div class="flex-1">
                    <div class="flex items-center space-x-2 mb-2">
                      <span class={`px-2 py-1 rounded text-xs font-medium ${getImportanceColor(memory.importance)}`}>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          <div class="flex items-center space-x-3">
            <div class="w-10 h-10 bg-blue-100 dark:bg-blue-900/30 rounded-lg flex items-center justify-center group-hover:bg-blue-200 dark:group-hover:bg-blue-800/40">
        ...
        >
          <div class="flex items-center space-x-3">
            <div class="w-10 h-10 bg-green-100 dark:bg-green-900/30 rounded-lg flex items-center justify-center group-hover:bg-green-200 dark:group-hover:bg-green-800/40">
              <span class="text-xl">📥</span>
            </div>
            <div class="text-left">
        ...
        >
          <div class="flex items-center space-x-3">
            <div class="w-10 h-10 bg-purple-100 dark:bg-purple-900/30 rounded-lg flex items-center justify-center group-hover:bg-purple-200 dark:group-hover:bg-purple-800/40">
              <span class="text-xl">📊</span>
            </div>
            <div class="text-left">
