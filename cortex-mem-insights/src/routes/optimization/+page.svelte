...
开始时间: ${reportData.start_time}
结束时间: ${reportData.end_time || '进行中'}
耗时: ${reportData.duration ? Math.floor(reportData.duration / 60) + '分钟' : '未知'}

发现问题: ${result.issues_found?.length || 0} 个
执行操作: ${result.actions_performed?.length || 0} 个
影响记忆: ${result.memories_affected || 0} 条
节省空间: ${result.space_saved_mb?.toFixed(2) || 0} MB
...
${reportData.logs?.join('\n') || '无日志'}
	...
	async function rollbackOptimization(jobId: string) {
		...
		try {
			// TODO: 实现撤销功能 - 需要后端支持
			// 目前只是删除历史记录
			errorMessage = '撤销功能暂未实现，该功能需要后端支持优化操作的回滚';
			console.log('撤销优化:', jobId);
		} catch (error) {
			console.error('撤销优化失败:', error);
			errorMessage = '撤销优化失败';
		}
	}

	async function clearOptimizationHistory() {
		if (!confirm('确定要清空所有优化历史记录吗？')) {
			return;
		}
		...
		try {
			...
			if (response.success) {
				...
				alert(response.data?.message || '历史记录已清空');
			}
		} catch (error) {
