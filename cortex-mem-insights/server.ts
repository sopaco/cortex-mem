/**
 * Cortex Memory Insights - Standalone Insights Host
 *
 * 这个文件是打包成可执行文件的入口点
...
function parseArgs(): {
  ...
  const apiIndex = args.findIndex(
    (arg) => arg === "--api-target" || arg === "--api",
  );
  ...
  // 禁用自动打开浏览器
  const noBrowser =
    args.includes("--no-browser") || args.includes("--headless");
...
`);
  ...
  if (!noBrowser) {
    ...
    console.log(
      `📝 Browser auto-open disabled. Please visit ${serverUrl} manually.`,
    );
  }
