...
graph TD
    ...
    subgraph "cortex-mem System"
        CLI[cortex-mem-cli]
        WEB[cortex-mem-insights\nWeb UI]
        HTTP[cortex-mem-service\nHTTP API]
        MCP[cortex-mem-mcp\nMCP Server]
    ...
    HTTP --> CORE
    MCP --> CORE
    CORE --> VSTORE[cortex-mem-core\nVector Store Adapter] --> Q
    CORE --> LLMCLIENT[cortex-mem-core\nLLM Client] --> L
    EVAL --> CORE
    EVAL --> HTTP
    EVAL --> WEB
...
> 🔒 **Boundary Rationale**: The system focuses on **memory orchestration**, not underlying AI model training or infrastructure management. This ensures focus on value-added logic while leveraging best-of-breed external systems.
...
| From → To | Mechanism | Protocol | Notes |
|---------|----------|--------|-------|
| UI → Service Layer | API Call | HTTP/JSON | Most common path; includes auth |
| UI → Core | Direct Library Use | Rust API | For CLI and examples only |
| Service Layer → Core | Function Call | Internal Rust API | All business logic executed here |
| Core → Storage | Adapter Pattern | HTTP/gRPC | Abstraction over Qdrant |
| Core → AI Processing | Async Request | HTTP/Streaming | Embedding generation |
| Evaluation → All | Test Orchestration | Direct Calls | Runs end-to-end tests |
...
| Capability | Method |
|----------|--------|
| Embedding Generation | `generate_embedding(text)` → `Vec<f32>` |
| Content Analysis | `analyze_content(memory)` → `{topics, summary, sentiment}` |
| Relevance Scoring | `score_relevance(query, memory)` → `f32` |
...
> → Embedding generated → Find top-5 similar stored memories → Return with confidence scores.
...
```
User → Interface → Service Layer → Core Engine → [LLM + Vector Store] → Response
```

#### Direct Access Path (CLI/Examples)
```
CLI → Core Engine → [LLM + Vector Store] → Result
```

#### Monitoring Path
```
Dashboard → System API → Aggregate Metrics → Health Checks → Visualization
```

#### Evaluation Path
```
Researcher → Experiment Runner → Load Dataset → Execute Queries → Collect Metrics → Report
```
...
> 📈 **Benchmark Target**: <100ms p95 latency for search under 10K memories.
...
> 💾 **Storage Note**: Qdrant requires sufficient RAM for vector index caching. For 1M vectors (768-dim), expect ~3GB RAM usage.
