...
- `cortex-mem-insights`：带有 Elysia API 后端的 SvelteKit Web 应用
...
flowchart TB
    ...
    subgraph Presentation["表示层"]
        Insights[cortex-mem-insights<br/>SvelteKit Web 仪表板<br/>Elysia API 服务器]
    end
...
- **关键能力**：系统健康监控、记忆统计可视化、优化任务管理、多语言支持（EN/CN/JP）
- **复杂度**：高（8/10）- 全栈 Web 应用复杂性
- **子模块**：洞察 API 服务器（Elysia）、Web 仪表板前端（SvelteKit）、前端状态管理、国际化和代理示例
...
**已识别的架构差距**：
1. **洞察直接集成**：`cortex-mem-insights` 目前绕过 REST API 进行某些操作，直接链接到 `cortex-mem-core`。*建议*：标准化所有洞察数据访问通过 `cortex-mem-service` 以保持边界完整性。
