...
AutomationConfig {
    ...
    index_on_close: false,    // Handled by exit-time sync
    index_batch_delay: 1,
}
```

#### 2.3.4 Exit-time Layer Generation & Sync

**Triggered by**: Application shutdown (`App::on_exit()`)
...
- `LayerGenerator::ensure_all_layers()` scans all directories
...
**Stage 3: Vector Sync**
- `SyncManager::sync_all()` scans entire filesystem
