...
1. **Progressive Threshold Reduction**: If L0 returns empty, reduce similarity threshold from 0.5 → 0.4 → 0.3
...
This background workflow automatically detects changes in filesystem-stored memory conversations and synchronizes them with the Qdrant vector database. It ensures real-time searchability while balancing performance through batch processing and deduplication.

#### 2.3.2 Execution Flow

```mermaid
flowchart TD
    A[File System Watcher] -->|Poll Interval 5s| B[Scan cortex://session]
    B --> C{Detect Changes}
    C -->|New/Modified| D[Queue for Batch Processing]
    C -->|No Changes| A
    
    D --> E[Batch Delay 2s]
    E --> F[Load Raw Content<br/>L2 Layer]
    
    F --> G[Check Hash Deduplication]
    G -->|Duplicate| A
    G -->|New Content| H[Generate L0 Abstract]
    
    H --> I[Generate L1 Overview]
    I --> J[Create Memory Objects]
    
    J --> K[Generate Vector ID<br/>URI + Layer Hash]
    K --> L[Embed with EmbeddingClient]
    
    L --> M[Upsert to Qdrant<br/>Tenant-aware Collection]
    M --> N[Update Index State]
    N --> A
```

#### 2.3.3 Key Mechanisms
...
- **Real-time Mode** (`index_on_message=true`): Immediate indexing on message add (high overhead)
- **Batch Mode** (default): Accumulates changes in `pending_sessions` set, processes via `tokio::select!` with timeout
