...
flowchart TB
    ...
    cortex-mem-insights --> Svelte
    cortex-mem-insights --> Elysia
...
- `cortex-mem-insights`: SvelteKit web application with Elysia API backend
...
flowchart TB
    ...
    subgraph Presentation["Presentation Layer"]
        Insights[cortex-mem-insights<br/>SvelteKit Web Dashboard<br/>Elysia API Server]
    end
...
- **Key Capabilities**: System health monitoring, memory statistics visualization, optimization task management, multilingual support (EN/CN/JP)
- **Complexity**: High (8/10) - Full-stack web application complexity
- **Sub-modules**: Insights API Server (Elysia), Web Dashboard Frontend (SvelteKit), Frontend State Management, Internationalization, Agent Examples
...
1. **Insights Direct Integration**: `cortex-mem-insights` currently bypasses the REST API for some operations, directly linking to `cortex-mem-core`. *Recommendation*: Standardize all Insights data access through `cortex-mem-service` to maintain boundary integrity.
